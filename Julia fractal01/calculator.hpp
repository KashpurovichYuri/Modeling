...
public:
    ...
    explicit Calculator(std::size_t width, std::size_t height, float real, float imaginary, std::size_t max_iterations) :
		m_width(width), m_height(height), m_c(real, imaginary), m_max_iterations(max_iterations)
	{
		initialize();
	}
